...
public static class IconDownloader {
    public static async Task<IEnumerable<IconInfo>> DownloadAndParseIconsAsync() {
            ...
            if (iconProperty.Value.TryGetProperty("svg", out var svgElement)) {
                foreach (var styleProperty in svgElement.EnumerateObject()) {
                    if (styleProperty.Value.TryGetProperty("raw", out var rawSvgElement)) {
                        ...
                        icons.Add(new IconInfo { Name = FontAwesomeIconGenerator.ToPascalCase(name), Data = xamlPath });
                        break;
                    }
                }
            }
