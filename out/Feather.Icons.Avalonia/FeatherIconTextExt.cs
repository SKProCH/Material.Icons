...
public class FeatherIconTextExt : FeatherIconExt {
    public FeatherIconTextExt() { }

    public FeatherIconTextExt(FeatherIconKind kind) : base(kind)
    {
    }

    public FeatherIconTextExt(FeatherIconKind kind, FeatherIconAnimation animation) : base(kind, animation)
    {
    }

    public FeatherIconTextExt(FeatherIconKind kind, double iconSize, FeatherIconAnimation animation = FeatherIconAnimation.None) : base(kind, iconSize, animation)
    {
    }

    public FeatherIconTextExt(FeatherIconKind kind, string text, FeatherIconAnimation animation = FeatherIconAnimation.None)
        : base(kind, animation)
    {
        Text = text;
    }

    public FeatherIconTextExt(FeatherIconKind kind, double iconSize, string text, FeatherIconAnimation animation = FeatherIconAnimation.None)
        : base(kind, iconSize, animation)
    {
        Text = text;
    }

    public FeatherIconTextExt(FeatherIconKind kind, double iconSize, Dock iconPlacement, string text, FeatherIconAnimation animation = FeatherIconAnimation.None)
        : base(kind, iconSize, animation) {
    ...
    public override object ProvideValue(IServiceProvider serviceProvider) {
        ...
        var result = new FeatherIconText();

        // Kind: binding takes precedence
        if (KindBinding is not null) result.Bind(FeatherIcon.KindProperty, KindBinding);
        else result.Kind = Kind;

        // Animation: binding takes precedence
        if (AnimationBinding is not null) result.Bind(FeatherIcon.AnimationProperty, AnimationBinding);
        else result.Animation = Animation;

        // Apply Text (supports binding or direct value)
        if (Text is IBinding textBinding) result.Bind(FeatherIconText.TextProperty, textBinding);
        else if (Text is string textValue) result.Text = textValue;

        if (IconSize is not null) {
            switch (IconSize) {
                case IBinding binding:
                    result.Bind(FeatherIcon.IconSizeProperty, binding);
                    break;
